...
JavaHighlighting::JavaHighlighting( QObject * parent )
  : QObject(parent), m_localColorization(true), m_useClassCache(false)
{
  generateColors(10);
}

JavaHighlighting::~JavaHighlighting( )
{
}

KTextEditor::Attribute::Ptr JavaHighlighting::attributeForType( Types type, Contexts context, uint color ) const
{
...
void JavaHighlighting::highlightDUChainSimple(DUContext* context) const
{
...
void JavaHighlighting::deleteHighlighting(KDevelop::DUContext* context) const {
  if (!context->smartRange())
    return;
...
void JavaHighlighting::highlightDUChain(DUContext* context, QHash<Declaration*, uint> colorsForDeclarations, ColorMap declarationsForColors) const
{
  if (!context->smartRange())
    return;
...
KTextEditor::Attribute::Ptr JavaHighlighting::attributeForDepth(int depth) const
{
...
KDevelop::Declaration* JavaHighlighting::localClassFromCodeContext(KDevelop::DUContext* context) const
{
  if(!context)
    return 0;
...
JavaHighlighting::Types JavaHighlighting::typeForDeclaration(Declaration * dec, DUContext* context) const
{
  /**
...
void JavaHighlighting::highlightDeclaration(Declaration * declaration, uint color) const
{
...
void JavaHighlighting::highlightDeclaration(Declaration * declaration) const
{
  if (SmartRange* range = declaration->smartRange())
    range->setAttribute(attributeForType(typeForDeclaration(declaration, 0), DeclarationContext, 0));
}

void JavaHighlighting::highlightUse(DUContext* context, int index, uint color) const
{
...
void JavaHighlighting::highlightUses(DUContext* context) const
{
  for(int a = 0; a < context->uses().count(); ++a) {
